...
export function switchWeapon(state, idx) {
	if (idx < 0) return; if (!state.player.weapons[idx]) return;
	state.player.curW = idx;
}

export function reload(state, bus) {
	const p = state.player, w = p.weapons[p.curW]; if (!w) return;
...
export function placeWallFront(state, bus) {
	const p = state.player; if (p.inv.blocks <= 0) { bus.emit('ui:toast', '資材がありません'); return; }
	const tx = Math.floor((p.x + p.facing.x * 18) / TILE), ty = Math.floor((p.y + p.facing.y * 18) / TILE);
	if (canPlaceAt(state, tx, ty)) {
		state.map[ty][tx] = '#'; state.tileHP[ty][tx] = state.tileMaxHP[ty][tx] = 70; p.inv.blocks--;
		bus.emit('ui:toast', '壁を設置'); bus.emit('sfx', 'build');
		// ※ flow 再計算は tiles.js 側でやると安全（後述）
	} else {
...
export function updateCombat(state, dt, bus, input, audio) {
	...
	// ★ 無敵時間を毎フレーム減らす（赤いまま＆ダメージ入らないバグ修正）
	...
	let len = Math.hypot(ax, ay) || 1; let vx = (ax / len) * spd * dt, vy = (ay / len) * spd * dt;
	...
	moveAndCollide(state, p, vx, 0); moveAndCollide(state, p, 0, vy);
	...
	if (input.pressed('j') && p.meleeCD <= 0) {
		...
		for (let oy = -1; oy <= 1; oy++) for (let ox = -1; ox <= 1; ox++) {
			const tx = ftx + ox, ty = fty + oy; if (tx < 0 || ty < 0 || tx >= state.dim.w || ty >= state.dim.h) continue;
	...
	if (input.pressed('k') && p.shootCD <= 0) {
		...
		if (w.id === 'beam') {
			if ((p.inv.ammoBeam || 0) <= 0) {
				...
				spawnBeamFX(state, p.x, p.y, ex, ey); // ★ 見た目
				bus.emit('sfx', 'beam');
			}
	...
	for (let i = state.bullets.length - 1; i >= 0; i--) {
		...
		if (state.map[ty] && (state.map[ty][tx] === '#' || state.map[ty][tx] === 'D')) { damageTile(state, tx, ty, b.dmg); state.bullets.splice(i, 1); continue; }
		let hit = false;
		for (const m of state.mobs) {
	...
	for (let i = state.grenades.length - 1; i >= 0; i--) {
		...
		if ((state.map[ty] && (state.map[ty][tx] === '#' || state.map[ty][tx] === 'D')) || g.fuse <= 0) {
			explode(state, g.x, g.y); state.grenades.splice(i, 1); continue;
		}
	}
	...
	for (let i = state.ebullets.length - 1; i >= 0; i--) {
		...
		b.x += b.vx * dt; b.y += b.vy * dt; b.life -= dt;
		const tx = Math.floor(b.x / TILE), ty = Math.floor(b.y / TILE);
		if ((state.map[ty] && (state.map[ty][tx] === '#' || state.map[ty][tx] === 'D')) || b.life <= 0) { state.ebullets.splice(i, 1); continue; }

		const hitP = (Math.abs(state.player.x - b.x) < state.player.w / 2 + 3) && (Math.abs(state.player.y - b.y) < state.player.h / 2 + 3);
		if (hitP) {
			if (p.isDashing) {
				const n = norm(b.x - p.x, b.y - p.y); b.vx = n.x * 260; b.vy = n.y * 260; b.life = Math.min(b.life, 0.9); continue;
			} else {
				if (p.iTime <= 0) {
					p.hp -= b.dmg; p.iTime = 0.8; const n = norm(p.x - b.x, p.y - b.y); p.vx += n.x * 180; p.vy += n.y * 180;
				}
...
function explode(state, x, y) {
	...
	if (dp < r * 0.7) {
		...
		const n = norm(state.player.x - x, state.player.y - y); state.player.vx += n.x * 200 * fall; state.player.vy += n.y * 200 * fall;
	}
	...
	spawnBlastFX(state, x, y, r); // ★ 爆発の見た目
}