...
class TouchButtonsTest {
    @Test fun `by default only dash, melee and weapon are shown`() {
        assertEquals(
            ...
            TouchButtons.visible(blocks = 0, mag = 30, magSize = 30),
        )
    }

    @Test fun `reload appears only when the magazine is not full`() {
        assertTrue(TouchButton.RELOAD in TouchButtons.visible(blocks = 0, mag = 5, magSize = 30))
    }

    @Test fun `wall appears only when materials are available`() {
        assertTrue(TouchButton.WALL in TouchButtons.visible(blocks = 3, mag = 30, magSize = 30))
    }

    @Test fun `an infinite-magazine weapon never shows reload`() {
        assertFalse(TouchButton.RELOAD in TouchButtons.visible(blocks = 0, mag = 0, magSize = null))
    }

    @Test fun `a full magazine never shows reload`() {
        assertFalse(TouchButton.RELOAD in TouchButtons.visible(blocks = 0, mag = 30, magSize = 30))
    }
}
