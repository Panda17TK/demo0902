...
class WildPredationSystem(private val mobGrid: SpatialGrid<Entity>) :
    ...
    // v2.164 軽い海: distance LOD — a far hunter's prey scan runs on a stagger
    private var tick = 0
    ...
    override fun onTickEntity(entity: Entity) {
        ...
        // v2.164 軽い海: a hunter beyond WildLod.MID cannot reach the keeper this frame anyway —
        // its bite scan joins the same stagger the rest of the far wild runs on.
        if (hasPl) {
            val lod = io.github.panda17tk.arpg.sim.WildLod.strideAt(t.x, t.y, plX, plY)
