...
data class PlayerCarry(
    val hp: Float, val stamina: Float, val blocks: Int, val dust: Int, val shards: Int, val wave: Int,
    ...
    fun applyTo(world: World, player: Entity) = with(world) {
        ...
        player[Materials].shards = shards
    ...
    companion object {
        fun of(world: World, player: Entity, wave: Int): PlayerCarry = with(world) {
            ...
            val ars = player[Arsenal]
            PlayerCarry(
                hp = player[Health].hp, stamina = player[Stamina].value, blocks = player[Materials].blocks, dust = player[Materials].dust, shards = player[Materials].shards, wave = wave,
