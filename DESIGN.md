...
### 敵キャラ描画（`render/enemy-sprites.js`）
- 種類ごとに固有シルエットと演出を持たせ、判別しやすくした：
  - **ゾンビ**: 角丸ボディ＋揺れる腕＋赤い落ち窪み目＋ジグザグの口、歩行で上下バウンド。
  - **スピッター**: 丸い粘体がぷるぷる脈動、体内に毒胞子ドット、前方に発射口、一つ目。
  - **ストーカー**: 向きに回転する菱形ボディ＋スリット模様＋鋭い一文字眼。
  - **中ボス/ボス**: 甲殻＋装甲プレート＋肩トゲ＋4つの複眼（狂乱で赤）、ボスは王冠、頭上に名前ラベル。
- 共通: 接地影、目が向き（`faceX/faceY`）に追従、個体ごとに位相をずらした歩行アニメ（`animSeed`）。
- 状態オーバーレイ（回避点滅・溜めテレグラフ・HPバー）は renderer 側に残し、本体描画と分離。

### 攻撃モーション・死亡演出・全体トーン統一
- **攻撃モーション**: 溜め近接中はゾンビ/ブルートが**腕を振り上げる**予備動作。射撃時は
  発射口が光る（スピッター/ボスの**砲門グロー** `m.fireFlash`、プレイヤーの**マズルフラッシュ＋反動** `p.muzzleT/recoil`）。
- **死亡演出**（`spawnDeathFX`）: 破片（`gib`、重力で弧を描く）＋着弾痕（`splat`）＋中心フラッシュ
  （`deathflash`）。エリートは破片が多く画面揺れ＋爆発音。
- **トーン統一**: プレイヤー（銃身・目・ダッシュ残像・接地影）、弾（進行方向トレイル＋白コア）、
  敵弾（脈動グロー、地雷は点滅）、グレネード（信管ランプ点滅）、アイテム（浮遊＋グロー＋接地影）を
  敵スプライトと同密度に底上げ。新FX: `muzzle/gib/splat/deathflash`。

### 被弾インジケータ・ボス撃破演出・環境
- **被弾方向インジケータ**（`recordPlayerHit` → `state.dmgMarks`）: ダメージ源の方向に画面端の
  赤い弧を表示。全被弾経路（敵弾・近接・slam・charge_melee・爆発）から記録。
  低HP（<30%）時は赤いパルスビネットも追加。
- **ボス撃破スローモーション**（`addSlowmo` + `state.killCam`）: エリート撃破で sim を減速
  （ボスは強め＆長め）、カメラを撃破地点へ寄せ、レターボックス＋「BOSS DOWN」表示。
  スロー減衰は実時間で行い演出が伸びない。
- **環境**: 背景に**2層パララックス**の塵（`drawParallax`、カメラに対し逆スクロール）、
  **床ディテール**（市松濃淡＋目地＋決定論ハッシュによる斑点／ひび＝チラつかない）。
